    ...
    if (itemProcessor.itemsToRespawn.length) {
      ...
      if (currentRoom.itemResetTimer > 3) {
        itemProcessor.itemsToRespawn.forEach(item => {
          ...
          if (item.respawnContents) {
            const arrayOfContents = item.respawnContents.map(respawn => newItem(respawn.category, respawn.name));
            respawnedItem.container.contains = arrayOfContents;
          }
          currentRoom.items.push(respawnedItem);
        });
        itemProcessor.itemsToRemove.forEach((item, index) => {
          currentRoom.items.splice(currentRoom.items.indexOf(currentRoom.items.find(_item => _item.id === item.id)), 1);
          itemProcessor.itemsToRemove[index] = undefined;
        });
