...
export default function putHandler(command, args, props) {
  ...
  const target = termsProcessor(props.inventory, splitArgs[1].split('.'));
  ...
  const argumentOfAll = splitArgs[0] === 'all';
  const validItems = argumentOfAll && target ? props.inventory.filter(item => item !== target) : null;
  ...
  const putItem = argumentOfAll ? 'all' : termsProcessor(props.inventory, splitArgs[0].split('.'));
